...
router.put('/announcements/:id', auth, requireRole('admin','manager','poc'), async (req, res) => {
  try {
    ...
    req.io?.emit('announcement:updated', result.rows[0])
    res.json({ announcement: result.rows[0] })
  } catch (err) { res.status(500).json({ error: 'Server error' }) }
})

// DELETE /api/poc/announcements/:id
router.delete('/announcements/:id', auth, requireRole('admin','manager','poc'), async (req, res) => {
  try {
